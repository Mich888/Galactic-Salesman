class Ship {
    ...
    constructor(name: string, capacity: number, planet: string) {
        this.name = name;
...
let ship1: Ship = new Ship("Axiom", 27, "Tatooine");
let ship2: Ship = new Ship("Enterprise", 46, "Corellia");
let ship3: Ship = new Ship("Goliath", 33, "Sur'Kesh");
let ship4: Ship = new Ship("Hermes", 26, "NowWhat");
let ship5: Ship = new Ship("Millenium Falcon", 35, "Tatooine");
let ship6: Ship = new Ship("Niezwyciężony", 60, "Argoland");
let ship7: Ship = new Ship("Normandy SR-2", 40, "Gaia");
let ship8: Ship = new Ship("Nostromo", 25, "Arrakis");
let ship9: Ship = new Ship("Rocinante", 30, "Alderaan");
let ship10: Ship = new Ship("Космонавт Алексе́й Лео́нов", 35, "Arrakis");

class Planet {
    name: string;
    commodities: Map<string, number>;
    ask: Map<string, number>;
...
function showShipPopup(ship: string) {
...
function showShipOnPlanet(ship) {
    ...
    planets.forEach((planet: Planet, planetName: string) => {
        if (ships.get(ship).planet != planetName) {
            let option = document.createElement("option");
            option.text = planetName;
            (<HTMLSelectElement>planetsToTravel).add(option);
        }
    });

    let commodityToBuy = document.getElementById("commodity_to_buy");
    commodityToBuy.innerHTML = "<option label='Towar:'>Towar:</option>";
    planets.get(shipLocation).commodities.forEach((amount: number, commodity: string) => {
        if (amount > 0) {
            let option = document.createElement("option");
            option.text = commodity;
            (<HTMLSelectElement>commodityToBuy).add(option);
        }
    });
    ...
    let commodities = "";
    ships.get(ship).commodities.forEach((amount: number, commodity: string) => {
        if (amount > 0) {
            let option = document.createElement("option");
            option.text = commodity;
            (<HTMLSelectElement>commodityToSell).add(option);
...
function showShipDuringJourney(ship: string) {
...
function showPlanetPopup(planetName: string) {
    ...
    let ships_on_planet = "";
    let ships_ref = <HTMLTableElement>document.getElementById("ships_on_planet");
    ships_ref.innerHTML = "";
    ships.forEach((ship: Ship, shipName: string) => {
    ...
    let commoditiesRef = <HTMLTableElement>document.getElementById("commodities_on_planet");
    commoditiesRef.innerHTML = "";
    let planet = planets.get(planetName);
    planet.commodities.forEach((amount: number, commodity: string) => {
        if (amount > 0) {
...
function displayShipCommodities(shipName: string) {
    let ship: Ship = ships.get(shipName);
}

function showShips() {
    let shipsRef = <HTMLTableElement>document.getElementById("ships23");
    shipsRef.innerHTML = "";
...
function buyCommodity() {
    ...
    let commodityToBuyRef = <HTMLSelectElement>document.getElementById("commodity_to_buy");
    let commodityToBuy = commodityToBuyRef.options[commodityToBuyRef.selectedIndex].text;
    console.log(commodityToBuy);
    let amountToBuy = parseInt((<HTMLInputElement>document.getElementById("amount_to_buy")).value);
...
function sellCommodity() {
    let commodityToSellRef = <HTMLSelectElement>document.getElementById("commodity_to_sell");
    let commodityToSell = commodityToSellRef.options[commodityToSellRef.selectedIndex].text;
    console.log(commodityToSell);
    let amountToSell = parseInt((<HTMLInputElement>document.getElementById("amount_to_sell")).value);
...
function countDistance(planet1: Planet, planet2: Planet) {
...
function beginJourney() {
    ...
    console.log(planet);
    let selectRef = (<HTMLSelectElement>document.getElementById("planet_to_travel"));
